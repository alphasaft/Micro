...
and apply it to `0`, what we do is to interpret the string literal ``` '`0` ``` (that will be generated by the parser as `"0"`) with `lift`. Since we return the string unchanged, it means we indeed want to see literals as pure strings, and then get an int out of it with `parseInt`. Here, it's not clear why we needed `lift` in the first place, as it does nothing particular and just returns the literal as-is, but this can come in handy sometimes. For example, you could want to tell the rest of your reducers that that thing they're receiving is indeed a literal :

```js
lift = s => { return { value: s, isLiteral: true }}
numberReducer = ($, s) => {
    let $s = $(s)
    if (!$s.isLiteral) throw "#number expects only literals as its arguments !"
    return parseInt($s.value)
}
```

And so on. `lift` is kind of a (really) advanced feature, but it's important to be aware it is there, and to know how to use it if needed. An example of another use case is given in [the syntactic typing section](#42-syntactic-typing).
